# 다른 프로젝트의 이슈를 현재 프로젝트로 복사하는 전체 예제 코드

이 예제는 **외부 프로젝트의 이슈(Source)**를 가져와 **현재 프로젝트(Target)**에 새로운 이슈로 복사하는 기능을 수행합니다.
특히 **대상 이슈 타입(Target Issue Type)에서 필수(Required)로 지정된 필드**를 동적으로 확인하여, 원본 이슈에서 해당 값을 찾아 채워 넣는 로직을 포함합니다.

## Java 구현 예제 (`IssueCopyHelper.java`)
...
public class IssueCopyHelper {
     ...
     */
    public Issue copyIssue(ApplicationUser user, String sourceIssueKey, String targetProjectKey, String targetIssueTypeId) {
        // 1. 원본 데이터 조회
        ...
        if (params.getSummary() == null) params.setSummary(sourceIssue.getSummary());
        if (params.getDescription() == null) params.setDescription(sourceIssue.getDescription());
     ...
     */
    private void populateFields(IssueInputParameters params, Issue sourceIssue, Project targetProject, String targetIssueTypeId) {
        // 대상 문맥의 필드 레이아웃 가져오기
        ...
        for (FieldLayoutItem item : items) {
            ...
            if (params.getActionParameters().containsKey(fieldId)) continue;
    ...
    private void copyField(IssueInputParameters params, Issue sourceIssue, String fieldId) {
        ...
        switch (fieldId) {
            ...
            case "priority":
                ...
                if (priority != null) params.setPriorityId(priority.getId());
                break;
            case "assignee":
                if (sourceIssue.getAssignee() != null) params.setAssigneeId(sourceIssue.getAssignee().getUsername());
                break;
            case "reporter":
                if (sourceIssue.getReporter() != null) params.setReporterId(sourceIssue.getReporter().getUsername());
                break;
            case "duedate":
...
1.  **필수 필드 데이터 누락**:
    ...
    - 만약 원본 이슈(Source)에는 값이 없는데(`null`), 대상 프로젝트(Target)에서는 해당 필드가 필수(`Required`)라면 생성에 실패합니다.
    - 이 경우 `validateCreate` 단계에서 에러가 잡히며, 로그에 `Field Error`로 나타납니다.
    - 해결책: `copyField` 메서드 내에서 원본 값이 `null`일 때 기본값(Default Value)을 넣어주는 방어 로직을 추가해야 합니다.

2.  **커스텀 필드 타입 호환성**:
    - 위 예제의 `value.toString()`은 텍스트 필드에는 작동하지만, `Select List`, `User Picker` 등 복잡한 객체에는 작동하지 않을 수 있습니다.
    - 커스텀 필드 값을 복사할 때는 `CustomFieldType`을 확인하여 `Option ID`나 `User Key`를 추출하는 상세 구현이 필요합니다.
